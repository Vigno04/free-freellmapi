...
describe('Tools-aware routing', () => {
  ...
  it('flags tool-capable families and leaves the known-bad ones unflagged', () => {
    ...
    const flag = (modelId: string) =>
      (db.prepare('SELECT modalities FROM models WHERE model_id = ?').get(modelId) as { modalities: string } | undefined)?.modalities?.includes('tools');

    // Verified tool-callers from the live benchmark stay eligible.
    expect(flag('openai/gpt-oss-120b')).toBe(true);
    expect(flag('gemini-2.5-flash')).toBe(true);
    expect(flag('llama-3.3-70b-versatile')).toBe(true);

    // hermes-3 emits tool calls as text — must NOT ride the llama-3 rule.
    const hermes = db.prepare("SELECT modalities FROM models WHERE model_id LIKE '%hermes-3%'").all() as { modalities: string }[];
    for (const h of hermes) expect(h.modalities.includes('tools')).toBe(false);

    // gemma must NOT ride the gemini rule.
    const gemma = db.prepare("SELECT modalities FROM models WHERE LOWER(model_id) LIKE '%gemma%'").all() as { modalities: string }[];
    for (const g of gemma) expect(g.modalities.includes('tools')).toBe(false);
  ...
  it('routeRequest skips non-tool models when requireTools is set', () => {
    ...
    const flag = db.prepare('SELECT modalities FROM models WHERE id = ?').get(tooled.modelDbId) as { modalities: string };
    expect(flag.modalities.includes('tools')).toBe(true);
