...
describe('GET /api/analytics/requests', () => {
  ...
  it('returns one row per call, newest first, with caller identity', async () => {
    const now = Date.now();
    const d1 = new Date(now - 3600000 * 3).toISOString().slice(0, 19).replace('T', ' ');
    const d2 = new Date(now - 3600000 * 2).toISOString().slice(0, 19).replace('T', ' ');
    const d3 = new Date(now - 3600000 * 1).toISOString().slice(0, 19).replace('T', ' ');

    insertCall(d1, '192.168.0.3', 'curl/8.6.0');
    insertCall(d2, '192.168.0.15', 'python-httpx/0.27');
    insertCall(d3, null, null);
    ...
    expect(body.rows[1].createdAt).toBe(d2.replace(' ', 'T') + 'Z');
  });

  it('paginates with limit/offset and clamps limit to 500', async () => {
    const now = Date.now();
    for (let i = 0; i < 5; i++) {
      const d = new Date(now - 3600000 * (5 - i)).toISOString().slice(0, 19).replace('T', ' ');
      insertCall(d, '10.0.0.1', 'ua');
    }
    ...
    const expectedD = new Date(now - 3600000 * 3).toISOString().slice(0, 19) + 'Z';
    expect(page.body.rows[0].createdAt).toBe(expectedD);
